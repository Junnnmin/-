...
let checkingBenefit = [{id:1,applied:false},{id:2,applied:false},{id:3,applied:false},{id:4,applied:false},{id:5,applied:false}];
...
function renderResults(matched, unmatched) {
  ...
  if (matched.length > 0) {
    matched.forEach((benefit) => {
      ...
      if (target) target.applied = checkbox.checked;

      console.log(checkingBenefit);
  ...
  async function saveCheckingBenefitToDB() {
      ...
      try {
        await setDoc(
          ...
          { checkingBenefit: checkingBenefit },
          { merge: true }
        );
